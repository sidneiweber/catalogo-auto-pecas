...
$sql = mysql_query("SELECT * FROM contas_pagar WHERE data LIKE '$busca' ORDER BY fornecedor ASC");
...
<div class="row">
          <div class="col-lg-12">
              <ol class="breadcrumb">
              <li class="active"><i class="fa fa-dashboard"></i> Busca de boletos</li>
            </ol>
          </div>
        </div><!-- /.row -->
...
while($res=mysql_fetch_array($sql)) {
	...
	$dia_hoje=date('d');
	$qr=mysql_query("SELECT SUM(valor) as total FROM contas_pagar WHERE data='$busca'");
